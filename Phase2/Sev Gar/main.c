...
int checkAuth(char *authTok){
    //printf("\nhere: %s\nhere: %s\nhere: %s\n", logArray[0].tokenu, logArray[1].tokenu, authTok);
    if(authTok[0] != 0)
...
void members(){
    ...
    if(userNum != -1){
        if(logArray[userNum].currChannelName[0]){
            ...
            cJSON_AddArrayToObject(listi, "content");
            for(int i=0; i<loggedInCount; i++){
                if(!strcmp(logArray[userNum].currChannelName, logArray[i].currChannelName)){
                    cJSON_AddItemToArray(cJSON_GetObjectItem(listi, "content"), cJSON_CreateString(logArray[i].name));
                }
            }
            strcpy(buffer, cJSON_Print(listi));
            cJSON_Delete(listi);
...
void leaveChan(){
    ...
    if(userNum != -1){
        if(logArray[userNum].currChannelName[0]){
            char *temp = strAdd(2, logArray[userNum].name, " left the channel.");
            addMessageToChannel("Server.l", temp,logArray[userNum].currChannelName);
            free(temp);

            logArray[userNum].currChannelName[0] = 0;
            logArray[userNum].currChannelSeenNum = 0;
            emptySuccessBuffer();
        }
        else{
...
void logOut(){
    ...
    if(userNum != -1){
        loggedInCount--;
        if(userNum < loggedInCount){
            strcpy(logArray[userNum].currChannelName, logArray[loggedInCount].currChannelName);
            logArray[userNum].currChannelSeenNum = logArray[loggedInCount].currChannelSeenNum;
            strcpy(logArray[userNum].name, logArray[loggedInCount].name);
            strcpy(logArray[userNum].tokenu, logArray[loggedInCount].tokenu);
        }
        else{
            logArray[userNum].currChannelName[0] = 0;
            logArray[userNum].currChannelSeenNum = 0;
            logArray[userNum].name[0] = 0;
            logArray[userNum].tokenu[0] = 0;
        }
        emptySuccessBuffer();
    }
    else{
...
void chat(int client_socket)
{
    ...
    while (true)
    {
        ...
        /*while(recv(client_socket, buffer, sizeof(buffer), 0) == -1)
            ;
        /*while(buffer[strlen(buffer)-1] != '\n'){
            recv(client_socket, tempBuff, sizeof(tempBuff), 0);
        ...
        else if(!strcmp(smallBuff, "channel")){
            sscanf(buffer, "%s %[^\n]s", smallBuff, buffer);
            members();
        }
        else if(!strcmp(smallBuff, "leave")){
            leaveChan();
        }
        else if(!strcmp(smallBuff, "logout")){
            logOut();
        }
...
int main()
{
    ...
    */
    SetConsoleTextAttribute(hStdOut, 10);
    printf("Server is now in manual mode\n");
